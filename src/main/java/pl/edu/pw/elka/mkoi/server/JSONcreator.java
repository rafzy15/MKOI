...
public class JSONcreator {
    ...
    public JSONObject createHashResponse(String responseType, String status, String hash) {
        JSONObject item = new JSONObject();
        item.put(Properties.MESSAGE_TYPE, responseType);
        item.put(Properties.MESSAGE_BODY, status);
        item.put(Properties.HASH_FILE, hash);
        return item;
    }

    public JSONObject createHashRequest(String requestType, String file, String login) {
        JSONObject item = new JSONObject();
        item.put(Properties.MESSAGE_TYPE, requestType);
